...
/**
 * Copied from Wordpress Core v5.7
 * Rev 50362
 */
...
/**
 * Cliptakes customised base class for displaying a list of items in an ajaxified HTML table.
 *
 * @since 3.1.0
 * @access private
 */
...
class CT_List_Table {
	...
	public function search_box( $text, $input_id ) {
		if ( empty( sanitize_text_field( $_REQUEST['s'] ) ) && ! $this->has_items() ) {
			return;
		}
		...
		$post_mime_type = sanitize_text_field( $_REQUEST['post_mime_type'] );
		if ( ! empty( $post_mime_type ) ) {
			echo '<input type="hidden" name="post_mime_type" value="' . esc_attr( $post_mime_type ) . '" />';
		}
		$detached = sanitize_text_field( $_REQUEST['detached'] );
		if ( ! empty( $detached ) ) {
...
<p class="search-box">
	...
	protected function pagination( $which ) {
		...
		echo wp_kses( $this->_pagination, $tagsInputIncluded );
	}
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( isset( $_POST['orderby'] ) ) {
			$current_orderby = sanitize_text_field( $_POST['orderby'] );
		} else {
			$current_orderby = '';
		}

		if ( isset( $_POST['order'] ) && 'desc' === sanitize_text_field( $_POST['order'] ) ) {
			$current_order = 'desc';
		} else {
			$current_order = 'asc';
		}
	...
	protected function single_row_columns( $item ) {
		...
		foreach ( $columns as $column_name => $column_display_name ) {
			...
			if ( 'cb' === $column_name ) {
				...
				echo "<td " . wp_strip_all_tags( $attributes ) . ">";
